...
This integration intends to facilitate cash-out operations between HiPay and Wordpress. This software is based on the [Silex PHP micro-framework](http://silex.sensiolabs.org/) and integrates the [HiPay Wallet cash-out connector for Wordpress][repo-lib] PHP library.
...
| Wordpress | Third-party and web service provider | Marketplace solution handling the shops, with a read-only web service |
| Cron | System service | Unix task scheduler, which may be replaced by a Windows alternative if needed (not tested with Windows) |
| Integrator | Human | The developer, who wishes to easily interface HiPay and Wordpress |
...
- Even though Wordpress does not require filling in the phone number section, it must be completed to create a HiPay Wallet account.
- Even though Wordpress only requires filling in the IBAN and the BIC in the banking information section, all form fields must be completed to provide banking information to HiPay.
...
# Next step
When you're done with this part, go to the next section: [[Wordpress account configuration]]

[repo-lib]: https://github.com/hipay/hipay-wallet-cashout-Wordpress-library
